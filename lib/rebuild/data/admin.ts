...
export async function fetchAdminDashboardData(supabase: DataClient, filters: AdminDataFilters): Promise<AdminDashboardData> {
  ...
  const [shiftRes, companyRes, boothRes, profileRes, categoryRes, subcategoryRes, linkRes, auditRes, punchRes, cashRes, closingRes, txRes, adjustmentRes] =
    await Promise.all([
      shiftQuery,
  ...
  const rows = (shiftRes.data as ShiftTotal[]) ?? [];
  const companies = (companyRes.data as Company[]) ?? [];
  const booths = (boothRes.data as Booth[]) ?? [];
  const profiles = (profileRes.data as Profile[]) ?? [];
  const categories = (categoryRes.data as Category[]) ?? [];
  const subcategories = (subcategoryRes.data as Subcategory[]) ?? [];
  ...
  const operatorBoothLinks = ((linkRes.data ?? []) as Omit<OperatorBoothLink, "profiles" | "booths">[]).map((link) => ({
    ...link,
  ...
  const auditLogs = ((auditRes.data ?? []) as Omit<AuditLog, "profiles">[]).map((audit) => ({
    ...audit,
    profiles: audit.created_by ? { full_name: profileMap.get(audit.created_by) ?? "-" } : null,
  }));

  const timePunchRows = ((punchRes.data ?? []) as Omit<TimePunchRow, "profiles" | "booths">[]).map((punch) => ({
    ...punch,
  ...
  const cashMovementRows = ((cashRes.data ?? []) as Omit<CashMovementRow, "profiles" | "booths">[]).map((movement) => ({
    ...movement,
  ...
  const shiftCashClosingRows = ((closingRes.data ?? []) as Omit<ShiftCashClosingRow, "profiles" | "booths">[]).map((closing) => ({
    ...closing,
  ...
  const reportTxs = ((txRes.data ?? []) as Omit<TxForReport, "profiles" | "booths" | "companies" | "transaction_categories" | "transaction_subcategories">[]).map(
    (transaction) => ({
      ...transaction,
  ...
  const adjustments = ((adjustmentRes.data ?? []) as Omit<Adjustment, "profiles" | "transactions">[]).map((adjustment) => {
